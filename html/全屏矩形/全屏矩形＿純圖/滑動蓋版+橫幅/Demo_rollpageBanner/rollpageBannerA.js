...
function handleAnimations(currentStep, currentProgress) {
  ...
  // Out 動畫處理
  const isInOutTriggerZone = (scrollDirection === 'down' && currentProgress >= 0.85) ||
                            (scrollDirection === 'up' && currentProgress <= 0.15);
  ...
  imgs.forEach((img, index) => {
    ...
    if (index < currentStep) {
      ...
      if (offset === 0) {
        ...
        basePosition = move;
        if (!animationStates.isOutAnimating && currentProgress >= 0.85) {
          basePosition = move - (currentProgress - 0.85) * move * 2;
        }
      }
...
function safariHacks() {
  ...
  window.addEventListener("resize", function() {
    ...
    bottomSpacing = Math.max(windowsVH * 2, 15);
    tenmaxLogo.style.bottom = `${bottomSpacing}px`;
    tenmaxLogo.style.setProperty("--vh", windowsVH + "px");
