var TenMaxScript = document.querySelector('#marqueeInterstitialPHILIPSJs');
...
let TenMaxTemplate = document.querySelector('#marqueeInterstitialPHILIPS');
...
let bannerClose = TenMaxTemplate.querySelector('.bannerClose');
...
interstitialClose.addEventListener('click', (e) => {
  e.preventDefault();
  stopRolling = false;
  startRolling();
  closeInterstitial();
...
cardA.addEventListener('transitionend', function(event) {
  if (cardA != event.target) {
    return;
  }
  cardA.classList.add('current');
  startRolling();
}, {once: true});
...
// 防止滑鼠事件影響其他 elements
TenMaxCards.addEventListener('dragstart', (e) => e.preventDefault());
...
function openInterstitial() {
	...
	bannerClose.classList.remove('show');
...
function closeInterstitial() {
	...
	bannerClose.classList.add('show');
  interstitialClose.classList.remove('show');
	pageBody.classList.remove('block');
}

function showContainer() {
  TenMaxTemplate.classList.add('show');
  TenMaxMarquee.classList.add('show');
  bannerClose.classList.add('show');
  TenMaxCards.querySelectorAll('.marqueeCard').forEach(card => {
